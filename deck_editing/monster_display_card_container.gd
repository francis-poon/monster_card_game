class_name MonsterDisplayCardContainer
...
func _can_drop_data(at_position: Vector2, data: Variant) -> bool:
	return data is TamedMonsterDisplayCard and data.get_parent() and data.get_parent() == self

func get_monster_list() -> Array:
	var monster_list: Array = []
	for child in get_children():
		if child is not TamedMonsterDisplayCard:
			continue
		child = child as TamedMonsterDisplayCard
